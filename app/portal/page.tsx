...
// ─────────────────────────────────────────────────────────────────────────────
// Landing NEUTRAL del portal de proveedores (rescatada de 60f8240, donde vivía
// en la raíz "/"). La raíz hoy es la landing COMERCIAL de Evolve BA; esta es la
// puerta de entrada sobria a la que apuntan los correos a proveedores
// ({urlPortal} en src/config/plantillasCorreo.ts).
//
// Server Component sin datos: getConfigEmpresa() es un helper puro (sin Prisma)
// y lo único dinámico es el check de sesión, que corre server-side vía auth().
// ─────────────────────────────────────────────────────────────────────────────
...
export default async function Portal() {
  ...
  if (session) {
    const tipo =
      (session.user as { tipoUsuario?: string })?.tipoUsuario ?? "comprador";
